...
// sendConfirmationEmail: sends a confirmation email with download link after successful merge
export async function sendConfirmationEmail(
	recipientEmail: string,
	downloadLink: string,
	priceDisplay: string = '$3.99'
): Promise<boolean> {
	try {
		...
		if (!sesClient) {
			console.log('[MOCK EMAIL] Would send confirmation to:', recipientEmail);
		...
		// HTML email body with download link
		const htmlBody = `
<!DOCTYPE html>
<html>
<head>
	...
	<title>SimpleMergePDF - Your Merged PDF is Ready</title>
</head>
		...
		<div style="background-color: #ffffff; padding: 40px 30px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
			...
			<div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin: 0 0 25px 0;">
				...
				<table style="width: 100%; border-collapse: collapse;">
					...
					<tr>
						...
						<td style="padding: 8px 0; color: #0f172a; font-size: 14px; text-align: right;">${priceDisplay}</td>
					</tr>
				</table>
			</div>
			...
			<div style="background-color: #f0fdfa; border: 1px solid #99f6e4; padding: 15px; border-radius: 8px; margin: 0 0 25px 0;">
				<p style="margin: 0; color: #065f46; font-size: 14px; line-height: 1.5;">
					<strong>Privacy Guarantee:</strong> Your merged PDF is stored temporarily for download. It will be deleted automatically after 30 days.
				</p>
			</div>
			...
			<p style="color: #94a3b8; font-size: 12px; line-height: 1.5; margin: 0; text-align: center;">
				This is an automated receipt from SimpleMergePDF.<br />
...
Privacy Guarantee: Your merged PDF is stored temporarily for download. It will be deleted automatically after 30 days.

If you have any questions or issues, please reply to this email.

---
This is an automated confirmation from SimpleMergePDF.
No subscription. No tracking. No data retention.
		`;
		...
		console.log('[EMAIL] Confirmation with download link sent to:', recipientEmail);
		return true;
	} catch (error) {
